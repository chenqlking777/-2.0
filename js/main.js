...
(function() {
  ...
  function updateProfile() {
    ...
    renderSocialLinks();
    // Set up avatar click for upload
      ...
      input.addEventListener("change", (e) => {
        ...
        if (file) {
          ...
          reader.onload = (ev) => {
            ...
            $("#hero-avatar-inner").innerHTML = "";
            $("#hero-avatar-inner").appendChild(img);
            localStorage.setItem("deom-avatar", ev.target.result);
          };
          reader.readAsDataURL(file);
        }
      });
      // Restore saved avatar
      const saved = localStorage.getItem("deom-avatar");
      if (saved) {
        const img = document.createElement("img");
        img.src = saved;
        $("#hero-avatar-inner").innerHTML = "";
