...
(function() {
  ...
  const SITE_BACKUP_KEY = "qiling-site-backup";
  ...
  function persistLocalContent(nextContent) {
...
window.editProject = function(id) {
    ...
    showToast("✅ 项目内容已保存到本地，发布请点“立即同步”");
  };
...
window.editPost = function(id) {
  ...
  window.savePost = function(id) {
    ...
    renderPostForm();
    persistLocalContent(content);
    showToast("✅ 日常内容已保存到本地，发布请点“立即同步”");
  };
      ...
      <div class="admin-card" style="margin-top:var(--space-lg);">
        ...
        <div class="admin-form" style="max-width:560px;">
          ...
          <div class="admin-form-actions">
            ...
            <button class="admin-btn" style="background:rgba(251,191,36,0.14);color:#fbbf24;" onclick="window.restoreLastBackup()">
              <i class="fas fa-rotate-left"></i> 恢复上次备份
            </button>
          </div>
          <div class="admin-sync-status ${escapeHtml(githubSyncState.status)}">
            <div><strong>当前状态：</strong>${getSyncStatusLabel()}</div>
            <div><strong>上次同步：</strong>${escapeHtml(githubSyncState.lastSyncAt || "暂无")}</div>
            <div><strong>本地备份：</strong>${getBackupTimeLabel()}</div>
          </div>
          <div class="admin-sync-log" id="admin-sync-log">
            ${renderSyncLogs()}
          </div>
          <p style="font-size:0.82rem;color:var(--admin-text-secondary);margin-top:12px;line-height:1.7;">
            ...
            建议先保存本地内容，再点击“立即同步”发布到网站。如果你同时在 GitHub 网页里改过同一个文件，后台会自动重试一次同步。
          </p>
        </div>
      </div>
    `;
  ...
  window.saveSettings = function() {
    ...
    persistLocalContent(content);
    showToast("✅ 设置已保存到本地，若需发布请点“立即同步”");
  };
  ...
  window.saveGitHubSyncConfig = function() {
    ...
    if (enabled && !/^[A-Za-z0-9._/-]+$/.test(branch)) {
      showToast("❌ 分支名格式不正确");
      return;
    }
  ...
  window.syncNow = async function() {
    ...
    try {
      setSyncStatus("running", "开始同步到 GitHub");
      await syncToGitHub();
      setSyncStatus("success", "同步完成");
  ...
  window.restoreLastBackup = function() {
    ...
    content = normalizeContent(backup.content);
    persistLocalContent(content);
    renderDashboard();
    renderProjects();
    renderPosts();
  ...
  function createBackup(nextContent) {
    const backup = {
      createdAt: new Date().toISOString(),
      content: JSON.parse(JSON.stringify(nextContent))
    };
    localStorage.setItem(SITE_BACKUP_KEY, JSON.stringify(backup));
    appendSyncLog("已创建同步前备份");
  }

  function getLastBackup() {
    try {
      const raw = localStorage.getItem(SITE_BACKUP_KEY);
      if (!raw) return null;
      return JSON.parse(raw);
    } catch (error) {
      return null;
  ...
  function getBackupTimeLabel() {
    const backup = getLastBackup();
    if (!backup?.createdAt) return "暂无";
  ...
  async function syncToGitHub() {
    ...
    try {
      ...
      const originalContent = JSON.parse(JSON.stringify(normalizeContent(content)));
      createBackup(originalContent);
      const uploads = [];
      const transformed = convertEmbeddedImages(originalContent, uploads);
      ...
      try {
        ...
        appendSyncLog("本地缓存写入失败，但 GitHub 已同步完成");
      }
  ...
  async function upsertGitHubFile(config, path, text, base64Content) {
    ...
    const contentValue = typeof text === "string" ? toBase64(text) : base64Content;
    if (!contentValue) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      if (response.status === 409 && attempt === 0) {
        appendSyncLog(`检测到 ${path} 版本冲突，正在自动重试`);
        continue;
      }
  ...
  function setSyncStatus(status, message) {
    githubSyncState.status = status;
    githubSyncState.lastSyncAt = new Date().toLocaleString("zh-CN");
    if (message) appendSyncLog(message);
  }

  function appendSyncLog(message) {
    githubSyncState.logs.unshift(`[${new Date().toLocaleTimeString("zh-CN")}] ${message}`);
    githubSyncState.logs = githubSyncState.logs.slice(0, 12);
    const logEl = document.getElementById("admin-sync-log");
    if (logEl) {
      logEl.innerHTML = renderSyncLogs();
    }
  }

  function renderSyncLogs() {
    if (!githubSyncState.logs.length) {
      return `<div class="admin-sync-log-empty">暂无同步日志</div>`;
    }
    return githubSyncState.logs.map((item) => `<div class="admin-sync-log-item">${escapeHtml(item)}</div>`).join("");
  }

  function getSyncStatusLabel() {
    if (githubSyncState.status === "running") return "同步中";
    if (githubSyncState.status === "success") return "同步成功";
    if (githubSyncState.status === "error") return "同步失败";
    return "未开始";
  }
